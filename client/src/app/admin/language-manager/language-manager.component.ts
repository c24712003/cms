  ...
  template: `
    <div class="max-w-7xl mx-auto">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <h1 class="text-2xl font-bold text-slate-800">{{ 'LANGUAGES_TITLE' | translate }}</h1>
      ...
      <div class="md:hidden space-y-4">
        <div *ngFor="let lang of languages()" class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            ...
            <div class="flex items-center justify-between pt-3 border-t border-slate-50">
                <div class="flex items-center gap-2">
                    <span class="text-xs text-slate-500 font-medium">{{ (lang.enabled ? 'STATUS_ENABLED' : 'STATUS_DISABLED') | translate }}</span>
                    <button (click)="toggleStatus(lang)" 
