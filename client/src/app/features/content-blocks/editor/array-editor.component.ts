...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, DragDropModule, forwardRef(() => PropertyPanelComponent), TranslatePipe],
  template: `
    <div class="space-y-4">
       <!-- Header -->
         ...
         <div *ngFor="let control of formArray().controls; let i = index" cdkDrag class="card bg-base-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm group">
            ...
            <div class="card-body p-4">
              <div class="flex items-center justify-between mb-3">
                 <div class="flex items-center gap-2 cursor-move text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300" cdkDragHandle>
                    ...
                    <span class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{{ 'BLOCK_PROP_ITEM_LABEL' | translate }} {{ i + 1 }}</span>
                 </div>
              ...
              <ng-template #primitiveInput>
                 <input [formControl]="asFormControl(control)" class="input input-sm input-bordered w-full dark:bg-slate-700 dark:border-slate-600 dark:text-slate-100" [placeholder]="'Value...' | translate" />
              </ng-template>
