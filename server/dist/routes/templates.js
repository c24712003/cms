...
const getTemplates = () => {
    try {
        if (!fs_1.default.existsSync(TEMPLATES_DIR))
            return [];
        const files = fs_1.default.readdirSync(TEMPLATES_DIR).filter(f => f.endsWith('.json'));
        return files.map(file => {
            const content = fs_1.default.readFileSync(path_1.default.join(TEMPLATES_DIR, file), 'utf-8');
...
router.post('/instantiate', async (req, res) => {
    ...
    try {
        ...
        const themeResult = await db.run('INSERT INTO themes (name, template_id, is_active) VALUES (?, ?, ?)', [themeName, templateId, isFirstTheme ? 1 : 0]);
        ...
        await (0, audit_logs_1.logActivity)('Theme Instantiated', `Instantiated theme "${themeName}" (${template.name})`, 'content');
