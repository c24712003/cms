...
router.post('/social/links', auth_1.authenticateToken, async (req, res) => {
...
router.post('/:code', auth_1.authenticateToken, async (req, res) => {
    ...
    try {
        // Validation
        const MAX_DEPTH = 3;
        const validateTree = (nodes, depth) => {
            if (depth > MAX_DEPTH)
                throw new Error(`Max menu depth of ${MAX_DEPTH} exceeded.`);
        ...
        const insertItems = async (nodes, parentId) => {
